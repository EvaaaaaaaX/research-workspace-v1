...
import { rankCandidates, getEmbedding } from "./src/lib/embedding-ranker";
...
async function runTest(title: string, content: string, type: string) {
  console.log(`\n==================================================`);
  ...
  const candidatesObj = extractCandidatePhrases(title, content);
  const candidates = candidatesObj.map(c => c.phrase);
  console.log(`Candidate Phrases count: ${candidates.length}`);

  // Step 2: Rank candidates
  const ranked = await rankCandidates(title, content, candidates);

  console.log(`Ranked Candidates (KeyBERT-like Cosine Similarity Ranking):`);
...
async function main() {
  console.log("Initializing local ONNX Embedding model (all-MiniLM-L6-v2)...");
  
  // Test a simple embedding to verify dimensions
  const testEmbed = await getEmbedding("test text");
  console.log(`Embedding Dimensions: ${testEmbed.length} (Expected: 384)`);

  await runTest(markdownTitle, markdownContent, "Markdown Document");
  await runTest(docxTitle, docxContent, "DOCX Document");
  await runTest(paperTitle, paperContent, "Academic Paper");
}

main().catch(err => {
  console.error("Error executing ranking test:", err);
});
